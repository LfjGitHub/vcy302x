...
typedef enum
{
    ...
    gaia_timer_language_confirm_time_s,             /* the time between EventUsrSelectAudioPromptLanguageMode and storing the language in ps */
    ...
    gaia_timer_store_current_peq_settings_timeout_s, /* the time in seconds elapsed after the last gaia command has been received to store eq settings into ps store */
...
static bool handleGaiaCommandSetVoiceRecognitionPhrase(GAIA_UNHANDLED_COMMAND_IND_T *command);
static bool handleGaiaCommandGetVoiceRecognitionPhrase(void);
...
static void gaia_alert_voice(uint8 payload_length, uint8 *payload)
{
    ...
    uint16 nr_prompts = SinkAudioPromptsGetNumAudioPromptFiles() / SinkAudioPromptsGetNumAudioPromptLanguages();
...
static void gaia_send_user_tone_config(uint8 id)
{
    ...
    else
    {
            ...
            while (*find != (unsigned) RINGTONE_END)
            {
...
static void gaia_send_tws_forced_downmix_mode(void)
{
    ...
    gaia_send_response(GAIA_VENDOR_CSR,
...
static bool gaia_handle_configuration_command(Task task, GAIA_UNHANDLED_COMMAND_IND_T *command)
{
    ...
    case GAIA_COMMAND_SET_VOICE_RECOGNITION_PHRASE:
        return handleGaiaCommandSetVoiceRecognitionPhrase(command);

    case GAIA_COMMAND_GET_VOICE_RECOGNITION_PHRASE:
        return handleGaiaCommandGetVoiceRecognitionPhrase();
...
static bool handleGaiaCommandSetVoiceRecognitionPhrase(GAIA_UNHANDLED_COMMAND_IND_T *command)
{
    if(command->size_payload == 1)
    {
        aov_set_phrase_response response = sinkAovSetPhraseIndex((signed char)command->payload[0]);

        switch(response)
        {
            case aov_set_phrase_response_success:
                gaia_send_success_payload(GAIA_COMMAND_SET_VOICE_RECOGNITION_PHRASE, 1, command->payload);
                return TRUE;

            case aov_set_phrase_response_invalid:
                /* Will send invalid parameter below */
                break;
                
            case aov_set_phrase_response_not_avail:
            default:
                return FALSE;
        }
    }
    gaia_send_invalid_parameter(GAIA_COMMAND_SET_VOICE_RECOGNITION_PHRASE);
    return TRUE;
}
...
static bool handleGaiaCommandGetVoiceRecognitionPhrase(void)
{
    int16 phrase_index = 0;
    if(sinkAovGetPhraseIndex(&phrase_index))
    {
        int8 index = phrase_index;
        gaia_send_success_payload(GAIA_COMMAND_GET_VOICE_RECOGNITION_PHRASE, 1, (uint8*)&index);
        return TRUE;
    }
    return FALSE;
}
...
void handleGaiaMessage(Task task, MessageId id, Message message)
{
    ...
    case GAIA_VA_DATA_REQUEST_IND:
        {
            GAIA_DEBUG(("G: GAIA_VA_DATA_REQUEST_IND\n"));
            SinkVaHandleDataReqInd();
        }
        break;
...
DESCRIPTION
    Request the GAIA host to start a Voice Assistant session
*/
void gaiaVoiceAssistantStart(void)
{
    GaiaVoiceAssistantStartRequest(gaia_get_transport());
}

/*************************************************************************
NAME
    gaiaVoiceAssistantEnd

DESCRIPTION
    Indicate to the Host that the Device has detected end of speech
*/
void gaiaVoiceAssistantEnd(void)
{
    GaiaVoiceAssistantVoiceEnd(gaia_get_transport());
}
...
DESCRIPTION
    Request the GAIA host to cancel a Voice Assistant session
*/
void gaiaVoiceAssistantCancel(void)
{
    GaiaVoiceAssistantCancelRequest(gaia_get_transport(), gaia_va_cancel_reason_user);
}
#endif /* ifdef ENABLE_VOICE_ASSISTANT */
