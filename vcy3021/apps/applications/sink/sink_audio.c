...
static uint16 audioPriorLatency(uint8 seid)
{
    ...
    if (getPeerRequiredStates() == RELAY_STATE_STREAMING)
    {
...
bool audioHfpPopulateConnectParameters(Sink sink, audio_connect_parameters *connect_parameters)
{
    ...
    if(sinkHfpDataGetAudioCodec(index))
    {
        ...
        connect_parameters->audio_plugin = AudioPluginVoiceVariantsGetHfpPlugin(sinkHfpDataGetAudioCodec(index), sinkHfpDataGetAudioPlugin());
    ...
    if(sink)
    {
        ...
        if(codec_settings)
        {
            ...
            if (codec_settings->codecData.latency_reporting) /* latency reporting retrieved from a2dp library */
            {
                ...
                audioConnectParams->latency.last = latency_ms / LATENCY_LAST_MULTIPLIER;
                audioConnectParams->latency.target = latency_ms / LATENCY_TARGET_MULTIPLIER;
    ...
    else
    {
        switch(id)
        {
            ...
            case EventUsr3DEnhancementEnableDisableToggle:
                sinkAudioSetEnhancement(MUSIC_CONFIG_SPATIAL_ENHANCE_BYPASS, toggleEnhancement(MUSIC_CONFIG_BASS_ENHANCE_BYPASS));
                break;
