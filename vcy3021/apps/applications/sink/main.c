...
static void handleUEMessage  ( Task task, MessageId id, Message message )
{
    ...
    {
         /* If Broadcast Audio is switching role, then Audio prompts shall be using the kalimba.
            Even before the switch takes place, if user events are triggered to use kalimba then 
            we run into panic. So, better we aviod any events when we are in process of switching
        
        if(!sinkBroadcatAudioAllowEvent(id))
        {
            MAIN_DEBUG(("HS: Broadcast Audio is busy switching role, can't support any events"));
            return;
        }
        */
    ...
    switch ( id )
    {
        ...
        case (EventUsrChargerDisconnected):
        {
            ...
            if (LedsInErrorMode() == led_err_id_enum_charger_error)
            {
                LedsIndicateError(led_err_id_enum_no_error);
            }
        ...
        case (EventSysChargerError):
        {
            ...
            LedsIndicateError(led_err_id_enum_charger_error);
        ...
        case EventSysBleBondableConnectionTimeout:
            {
                ...
                sinkSportHealthSMDeviceStateCheck(EventSysBleBondableConnectionTimeout);
            }
            break;
       ...
       case EventUsrFindMyPhoneImmAlertMild:
        ...
        case EventUsrCycleAovPhrase:
            MAIN_DEBUG(("Cycling AOV service"));
            sinkAovCyclePhrase();
            break;
      ...
      case EventSysCodecAptxAdaptiveInUse:
        ...
        case EventUsrVoiceAssistantStart:
            SinkVaStartSessionEvent();
            break;
           
        case EventUsrVoiceAssistantCancel:
...
#endif
{
    ...
    status = ConfigStoreInit(ConfigDefinitionGetConstData(), 
                             ...
                             sinkUpgradeEnableConfigStoreConstMode());
